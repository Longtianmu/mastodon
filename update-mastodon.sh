#!/usr/bin/env bash
# Deploy versioned custom Mastodon branches from the "mod" Git remote.
# Run --plan first on production.

set -Eeuo pipefail

REMOTE="${MASTODON_MOD_REMOTE:-mod}"
TARGET=""
MOD_BRANCH_PREFIX="${MASTODON_MOD_BRANCH_PREFIX:-mod2-v}"
...
Safely switches to the newest versioned custom branch from the mod remote.
...
Options:
  ...
  --target BRANCH         Deploy this exact custom branch (or mod/BRANCH)
  --from-version X.Y.Z    Deployed/database version (default: 4.3.8)
  --remote NAME           Remote containing modified branches (default: mod)
  --no-fetch              Use existing local/remote-tracking modified branches
...
Examples:
  ...
  ./update-mastodon.sh --target 4.7.0 --yes
...
Environment overrides for native deployments:
  ...
  MASTODON_MOD_REMOTE     Git remote with modified branches (default: mod)
  MASTODON_MOD_BRANCH_PREFIX
                          Version-to-branch prefix (default: mod2-v)
EOF
}
...
list_modified_branches() {
  ...
  while IFS= read -r ref; do
    ...
    printf '%s\t%s\n' "$version" "$branch"
  done < <(git for-each-ref --format='%(refname:short)' "refs/remotes/$REMOTE/${MOD_BRANCH_PREFIX}*")

  while IFS= read -r ref; do
    version="$(extract_release_version "$ref")" || continue
    printf '%s\t%s\n' "$version" "$ref"
  done < <(git for-each-ref --format='%(refname:short)' "refs/heads/${MOD_BRANCH_PREFIX}*")
}
...
Release requirements for Mastodon 4.7.x:
...
EOF
  ...
  if [[ "$DEPLOY" == "native" ]]; then
    ...
    version_at_least "$ruby_version" "3.3" || die "Ruby $ruby_version is too old; Mastodon 4.7 needs Ruby >= 3.3"
    version_at_least "$node_version" "22" || die "Node $node_version is too old; Mastodon 4.7 needs Node >= 22"
...
if [[ -z "$TARGET" ]]; then
  ...
  [[ -n "$TARGET_BRANCH" ]] || die "No branches matching ${MOD_BRANCH_PREFIX}X.Y.Z found for remote '$REMOTE'"
...
[[ "$TARGET_VERSION" =~ ^4\.[67]\.[0-9]+$ ]] \
  || die "This migration recipe has been reviewed through Mastodon 4.7.x only. Read the new minor/major release notes and update this script before targeting $TARGET_VERSION."
...
Database path: v$FROM_VERSION -> v$TARGET_VERSION (review every skipped x.y.0 release)
...
Mastodon 4.7 has unusually long post-deployment migrations; large instances may
need a few hours. Do not interrupt them. Zero-downtime migration is guaranteed
only when the deployed version is already on Mastodon 4.6.
